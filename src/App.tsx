          ...
          <div className="flex h-full flex-col-reverse lg:flex-row gap-4 lg:gap-6 xl:gap-8">
            <div className="flex w-full lg:w-[38%] xl:w-[36%] flex-col-reverse lg:flex-col gap-4 lg:gap-5">
              <GlassPanel title="CONTROL SURFACE" subtitle="MISSION CONTROL PANELS">
                <div className="flex flex-col gap-3 sm:gap-4">
                  <DashboardStats
                    ...
                    activeHelpersCount={helperSim.activeHelpers.length}
                    totalHelpersCount={helpers.length}
                    isHelperSimRunning={helperSim.running}
                    autoSim={autoSim}
                  />
                  <ControlButtons
                    createBatchOrders={createBatchOrders}
                    advanceOrders={advanceOrders}
                    autoSim={autoSim}
                    setAutoSim={setAutoSim}
                    resetOrders={resetOrders}
                    isHelperSimRunning={helperSim.running}
                    toggleHelperSim={helperSim.toggleRunning}
                    stressTestMode={stressTestMode}
                    setStressTestMode={setStressTestMode}
                    homeMode={homeMode}
                    setHomeMode={setHomeMode}
                    laundryFailureMode={laundryFailureMode}
                    setLaundryFailureMode={setLaundryFailureMode}
                  />
                  <RecentOrdersList
                    ...
                    helperIntegrity={helperIntegrity}
                    helperSopChecklist={helperSopChecklist}
                    onOrderClick={(id) => setMediaModalOrderId(id)}
                  />
                  <CompletedOrdersList
                    completedOrders={completedRecentOrders}
                    houseClients={houseClients}
                    laundryLocations={laundryLocations}
                    helperSopChecklist={helperSopChecklist}
                    homeMode={homeMode}
                  ...
                  <AssetProtectionPanel
                    ...
                    latestTrunkUnlocked={latestTrunkUnlocked}
                    latestAssetOrderId={latestAssetOrderId}
                    serviceException={serviceException}
                    mechanicalOverrideEnabled={mechanicalOverrideEnabled}
                    latestRescueEtaLabel={latestRescueEtaLabel}
                    latestRescueAssignment={latestRescueAssignment}
                    latestRoutingAssignments={latestRoutingAssignments}
                  />
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <HelperStatusList
                      activeHelpersCount={helperSim.activeHelpers.length}
                      totalHelpersCount={helpers.length}
                      helpers={helperSim.helpers}
                      helperIntegrity={helperIntegrity}
                      laundryLocations={laundryLocations}
                      homeMode={homeMode}
                      now={now}
                    />
                    <HelperEventsList logs={helperSim.logs} />
                  </div>
                </div>
              </GlassPanel>
              ...
              <DecisionCenter
                ...
                totalOrders={totalOrders}
                completedOrders={completedOrders}
                handleGenerateSimulationOrder={handleGenerateSimulationOrder}
